...
def post_transaction(tx_type, partner_id, amount):
    ...
    tx_id = query(
        ...
        (tx_type, partner_id, amount, datetime.now().strftime("%Y-%m-%d %H:%M")),
        commit=True,
    )
    ...
    entries = [
        (
            tx_id,
            p_acc,
            amount if p_side == "dr" else 0,
            amount if p_side == "cr" else 0,
        ),
        (
            tx_id,
            o_acc,
            amount if o_side == "dr" else 0,
            amount if o_side == "cr" else 0,
        ),
    ]
